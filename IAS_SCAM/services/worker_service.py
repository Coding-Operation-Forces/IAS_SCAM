...
from db.models import (Requests, Applicants, RequestDetails, 
                       IssueType, Category, Status, Materials, StatusHistory) 
    ...
    with SessionLocal() as db:
        try:
            ...
            def request_sorting_key(r):
                ...
                if "нова" in s_name: s_weight = 1
                elif "робот" in s_name: s_weight = 2
                else: s_weight = 3
                
                if "критич" in c_name: c_weight = 1
                elif "висок" in c_name: c_weight = 2
                elif "середн" in c_name: c_weight = 3
                else: c_weight = 4
            ...
            for r in requests:
                ...
                result.append({
                    ...
                    "description": r.description or "Немає опису",
        ...
        try:
            for u in updates:
                ...
                if req:
                    ...
                    if 'crew_id' in u: req.crew_id = u['crew_id'] if u['crew_id'] != 0 else None
            db.commit()
    ...
    with SessionLocal() as db:
        try:
            ...
            if applicant:
                ...
                if applicant.house_number:
                    full_street += f" {applicant.house_number}"
                return {
                    "found": True, "applicant_id": applicant.id_applicant,
                    "lname": applicant.last_name or "", "fname": applicant.first_name or "",
                    "mname": applicant.patronymic or "", "phone": applicant.phone or "",
        ...
        try:
            ...
            if not applicant_id:
                new_applicant = Applicants(
                    account_number=app_data["account"], last_name=app_data["lname"],
                    first_name=app_data["fname"], patronymic=app_data["mname"],
                    phone=app_data["phone"], email=app_data["email"],
                    city="Київ",
                    street=street, house_number=house_number, 
                    entrance=entrance_val, apartment=app_data["apartment"], floor=floor_val
                )
            ...
            new_request = Requests(
                request_date=datetime.now(), applicant_id=applicant_id,
                issue_type_id=req_data["issue_type_id"], criticality_id=req_data["criticality_id"],
                status_id=default_status.id_status, channel=req_data["channel"],
            ...
            first_history = StatusHistory(
                request_id=new_request.id_request,
...
def get_used_materials_report():
    with SessionLocal() as db:
        try:
            ...
            # ПЕРЕНЕСЕНО НА ФРОНТЕНД: Форматування рядків звіту
            return [{
                "id": r.id_detail,
...
def write_off_material(request_id, material_id, quantity):
    with SessionLocal() as db:
        try:
            if not db.query(Requests).filter(Requests.id_request == request_id).first(): return False, "Заявку не знайдено!"
            material = db.query(Materials).filter(Materials.id_material == material_id).first()
            if not material: return False, "Матеріал не знайдено!"
            qty_float = float(quantity)
            usage = RequestDetails(request_id=request_id, material_id=material_id, quantity=qty_float, total_cost=float(material.price) * qty_float)
            db.add(usage)
            db.commit()
            return True, "Матеріал успішно додано!"
        except Exception as e: return False, f"Помилка: {str(e)}"
