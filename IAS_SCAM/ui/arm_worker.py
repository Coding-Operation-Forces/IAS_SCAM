...
class ArmWorkerWindow(BaseArmWindow):
    ...
    def get_help_data(self):
        return {
            "Головне про модуль": """
                ...
                <p>Основне робоче місце для оперативного прийому заявок від населення, маршрутизації їх до ремонтних бригад та списання використаних матеріалів.</p>
            """,
            "Перегляд та обробка заявок": """
                <h3>Диспетчеризація</h3>
                <p>Таблиця містить всі заявки системи. Дані оновлюються автоматично кожні 30 секунд (або вручну кнопкою "Оновити").</p>
                <ul>
                    <li><b>Швидке редагування:</b> Змінюйте "Статус" та "Бригаду" прямо у випадаючих списках всередині таблиці. Обов'язково натисніть <b>"💾 Зберегти зміни"</b>, щоб записати їх у базу даних!</li>
                    <li><b>Детальний опис:</b> Зробіть <b>подвійний клік</b> по колонці "Опис проблеми", щоб зручно прочитати повний текст звернення.</li>
                    <li><b>Контекстне меню:</b> Клікніть <b>правою кнопкою миші</b> по заявці, щоб побачити її на карті або швидко перейти до списання матеріалів по ній.</li>
                    <li><b>Фільтрація:</b> Вводьте текст у поля під заголовками колонок для швидкого пошуку потрібної адреси чи заявника.</li>
                </ul>
            """,
            "Реєстрація звернень": """
                <h3>Створення нової заявки</h3>
                <p>Процес поділено на дві логічні частини: дані заявника та деталі самої аварії.</p>
                <ul>
                    <li><b>Автозаповнення:</b> Введіть номер особового рахунку та натисніть "🔍 Знайти". Якщо абонент є в базі, всі його дані (включаючи адресу та квартиру) підтягнуться автоматично.</li>
                    <li><b>Ручне введення:</b> Якщо абонент новий, змініть "Тип адреси" на ручний. Почніть вводити назву вулиці міста Києва — система запропонує <b>автодоповнення</b> з офіційного реєстру.</li>
                    <li><b>Опис:</b> Обов'язково вкажіть категорію, тип аварії, рівень її критичності та опишіть проблему словами мешканця.</li>
                </ul>
            """,
            "Виконання робіт": """
                <h3>Списання матеріалів</h3>
                <p>Після фактичного виконання ремонту диспетчер або майстер фіксує витрачені ТМЦ (товарно-матеріальні цінності).</p>
                <ul>
                    <li>Впишіть ID заявки (можна подивитись у першій вкладці).</li>
                    <li>Оберіть матеріал із системного довідника. Поруч відображається ціна за одиницю.</li>
                    <li>Вкажіть витрачену кількість (можна використовувати дробові числа, наприклад, 1.5 метра труби).</li>
                    <li>Натисніть "Додати до звіту". Сума вирахується автоматично, а дані потраплять у фінансовий звіт Керівника.</li>
                </ul>
            """
        }
    ...
    def toggle_requests_view_mode(self):
        ...
        if self.show_all_mode:
            ...
            self.btn_toggle_view.setText("Показати всі заявки")
        self.refresh_requests_table()
    ...
    def show_table_context_menu(self, pos: QPoint):
        ...
        act_map = menu.addAction("📍 Показати в окремому вікні на карті")
        act_works = menu.addAction("🔧 Заповнити у виконання робіт")
